/**
 * ArtistAssistApp
 * Copyright (C) 2023-2026  Eugene Khyst
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <http://www.gnu.org/licenses/>.
 */

import {Trans} from '@lingui/react/macro';
import {Button} from 'antd';
import type {ButtonProps} from 'antd/lib';

import {CLOUD_PROVIDER_LABELS} from '@/components/messages';
import {useAppStore} from '@/stores/app-store';
import {CloudOperationType} from '@/stores/cloud-slice';

export function SyncCloudButton({...rest}: Readonly<Pick<ButtonProps, 'type'>>) {
  const provider = useAppStore(state => state.cloudConnection?.provider);
  const cloudOperation = useAppStore(state => state.cloudOperation);
  const syncCloudState = useAppStore(state => state.syncCloudState);

  if (!provider) {
    return null;
  }
  const label = CLOUD_PROVIDER_LABELS[provider];
  const isSyncing = cloudOperation?.type === CloudOperationType.Sync;
  return (
    <Button
      loading={isSyncing}
      disabled={!!cloudOperation && !isSyncing}
      onClick={() => {
        void syncCloudState();
      }}
      {...rest}
    >
      <Trans>Sync with {label}</Trans>
    </Button>
  );
}
