...
import highPassFragmentShaderSource from './glsl/high-pass.glsl';

export function highPassWebGL(
  ...
  const renderer = new WebGLRenderer(
    [gaussianBlurFragmentShaderSource, highPassFragmentShaderSource],
    [
      ['u_kernel', 'u_kernelSize', 'u_direction'],
      ['u_original', 'u_contrast'],
    ],
    image,
  ...
  const renderPasses: RenderPass[] = [
    ...gaussianBlurRenderPasses(kernelSize, 0, standardDeviation),
    {
      ...
      setUniforms(gl, locations) {
        gl.uniform1f(locations.get('u_contrast')!, contrast);
      },
    },
  ];
  try {
