...
import {FREE_CROP_ASPECT_RATIO, ORIGINAL_CROP_ASPECT_RATIO} from '@/services/image/aspect-ratio';
...
} from '@/stores/correct-perspective-slice';
...
vi.mock('@/services/image/correct-perspective', () => ({
...
function createTestStore(image: ImageBitmap | null = null) {
  ...
  const store = createStore<TestStore>()((...args) => ({
    ...
    ...createCorrectPerspectiveSlice(...args),
  }));
...
describe('discrete image-editing slices', () => {
  it('persists and resets to the stored Crop aspect ratio', () => {
  ...
  it('loads the Crop aspect ratio from settings', () => {
    const {store, saveAppSettings} = createTestStore();
  ...
  it('loads the free Crop aspect ratio from settings', () => {
    const {store} = createTestStore();

    store.getState().loadCropSettings({
      ...DEFAULT_APP_SETTINGS,
      cropAspectRatio: FREE_CROP_ASPECT_RATIO,
    });

    expect(store.getState().cropAspectRatio).toBeNull();
  });
  ...
  it('executes Correct perspective as a cumulative command', () => {
    const {store, execute} = createTestStore();
    const vertices = [new Vector(1, 2), new Vector(3, 4), new Vector(5, 6), new Vector(7, 8)];

    store.getState().correctPerspectiveImage(vertices);
  ...
  it('does not auto-detect without a model', async () => {
    ...
    await expect(store.getState().autoDetectCorrectPerspectiveVertices()).resolves.toBeNull();
    expect(withEditedImage).not.toHaveBeenCalled();
  });

  it('does not auto-detect without access to the model', async () => {
    const {store, withEditedImage} = createTestStore(createImage());
    store.getState().setCorrectPerspectiveModel(onnxModel(false));
  ...
  it('auto-detects with a free-tier model', async () => {
    ...
    store.getState().setCorrectPerspectiveModel(model);

    await expect(store.getState().autoDetectCorrectPerspectiveVertices()).resolves.toBe(vertices);
  ...
  it('discards vertices detected by a model that is no longer selected', async () => {
    ...
    store.getState().setCorrectPerspectiveModel(onnxModel(true));
    imageOperations.detectDocumentCorners.mockImplementationOnce(() => {
      store.getState().setCorrectPerspectiveModel(onnxModel(true));
      return Promise.resolve([new Vector(0, 0)]);
    });

    await expect(store.getState().autoDetectCorrectPerspectiveVertices()).resolves.toBeUndefined();
  });
});
