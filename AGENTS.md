...
Service worker at `src/service-worker.ts`, initialized via `src/pwa-init.ts`. Cross-Origin headers
(COEP/COOP) are required for SharedArrayBuffer support (ONNX WASM threading). The SW intercepts
`POST /login/callback` (auth — see `auth/` above) and `POST /share-target` (file sharing). All
persistence uses IndexedDB — `localStorage` is not used.
