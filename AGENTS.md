...
- `npm run build && npm run preview` — build + preview the production bundle with the service worker
  active. Use `build:local-dev` instead when testing against local auth + data servers.
...
then switch to `dev` for hot-reload development.
...
underneath — max contrast on any surface, no WebGL pass. Distinct from the grid/overlay
pre-inversion (`invert-colors-webgl.ts` + `invert-colors.glsl`).
...
- **`image/`** — `outline.ts` dispatches on `OnnxModel.url` (ONNX vs. local Sobel pipeline);
  `perspective-correction.ts` does margin-restricted argmax + weighted PCA with outlier rejection
  (not naive Hough); `sampling-point.ts` picks deepest pixel per region via Chamfer 3-4 distance
  transform; `color-match.ts` reuses the Sobel+Otsu+threshold pipeline.
...
- **`auth/`** — JWT auth client using OIDC Form Post Response Mode. Auth server POSTs `id_token`,
  ...
  `AuthErrorHandler` shows the matching modal. Two callback interception paths:
  ...
  2. **CF Pages Function fallback** (`functions/login/callback.ts`) — handles the POST when the SW
     ...
     `data-auth-callback` attribute on `<body>`, then serves `index.html`. The client reads from
     `document.body.dataset.authCallback` first, falling back to URL params. CF Function types live
     in `functions/types.d.ts` (generated by `cf-typegen`); `functions/` has its own `tsconfig.json`
...
Service worker at `src/service-worker.ts`, initialized via `src/pwa-init.ts`. `vite-plugin-pwa` with
`injectManifest` strategy. Cross-Origin headers (COEP/COOP) are required for SharedArrayBuffer
support (ONNX WASM threading). The SW intercepts `POST /login/callback` (auth — see `auth/` above)
